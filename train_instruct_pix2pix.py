...
# check_min_version("0.36.0.dev0")  # Commented out to use stable version 0.35.x
...
def download_image(url_or_path):
    ...
    if url_or_path.startswith(('http://', 'https://')):
        ...
        image = PIL.Image.open(requests.get(url_or_path, stream=True, timeout=DIFFUSERS_REQUEST_TIMEOUT).raw)
    else:
...
def main():
    ...
    with torch.no_grad():
        # Check if the model already has 8 input channels (trained InstructPix2Pix)
        if unet.conv_in.in_channels == 8:
            logger.info("Model already has 8 input channels, skipping weight initialization.")
        else:
            # Original initialization logic for converting 4-channel model to 8-channel
    ...
    if args.dataset_name is not None:
        ...
        if args.train_data_dir is not None:
            ...
            if train_annotations_path.exists():
                ...
                from data.sthv2_dataset_hf import create_sthv2_instruct_pix2pix_dataset
        ...
        if accelerator.is_main_process:
            if (
                ...
                try:
                    ...
                    logger.warning(f"Validation failed: {e}. Continuing training...")
